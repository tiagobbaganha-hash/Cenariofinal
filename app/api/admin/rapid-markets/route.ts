...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const { data: market } = await supabase.from('markets')
      .select('id, rapid_config, closes_at, status').eq('id', market_id).single()
    ...
    if (!config) return NextResponse.json({ error: 'Sem configuração rapid' }, { status: 400 })
    ...
    const { data: winOption } = await supabase.from('market_options')
      .select('id').eq('market_id', market_id).eq('option_key', winningKey).single()
